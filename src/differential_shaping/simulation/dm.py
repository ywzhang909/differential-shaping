...
from ..params import D, n_act_x, n_act_y, n_act, act_spacing, sigma_inf, N
...
def generate_actuator_grid() -> tuple[torch.Tensor, torch.Tensor]:
    ...
    xs = torch.linspace(-D / 2 + margin, D / 2 - margin, n_act_x,
                         dtype=_DTYPE, device=_DEVICE)
    ys = torch.linspace(-D / 2 + margin, D / 2 - margin, n_act_y,
                         dtype=_DTYPE, device=_DEVICE)
